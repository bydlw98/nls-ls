...
impl Account {
    pub fn get_by_uid(uid: u32, config: &Config) -> Self {
        if config.numeric_uid_gid {
            Self {
                name_cell: DisplayCell::from_ascii_string(uid.to_string(), true),
                numeric_id: uid,
            }
        } else {
            ...
            unsafe {
                ...
                if return_code == 0 && (result == &mut pwd) {
                    ...
                    Self {
                        name_cell: DisplayCell::from(username_string),
                        numeric_id: uid,
                    }
                } else {
                    Self {
    ...
    pub fn get_by_gid(gid: u32, config: &Config) -> Self {
        if config.numeric_uid_gid {
            Self {
                name_cell: DisplayCell::from_ascii_string(gid.to_string(), true),
                numeric_id: gid,
            }
        } else {
            ...
            unsafe {
                ...
                if return_code == 0 && (result == &mut grp) {
                    let groupname_string =
                        CStr::from_ptr(grp.gr_name).to_string_lossy().to_string();

                    Self {
                        name_cell: DisplayCell::from(groupname_string),
                        numeric_id: gid,
                    }
                } else {
                    Self {
